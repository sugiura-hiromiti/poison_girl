...
///
/// ```rust,ignore
/// use poison_girl_kernel::base::io::TextBuf;
///
...
// 		let mut ___num = [0; poison_girl_kernel::base::text::MAX_DIGIT];
...
// from the `poison_girl_proc_macro` crate.
impl_int!(u8, u16, u32, u64, u128, usize, i8, i16, i32, i64, i128, isize);

#[cfg(test)]
mod tests
{
	use {super::Integer, core::prelude::rust_2024::test};
	...
	#[test]
	fn unsigned_shift_right_returns_rightmost_digit_and_truncates()
	{
		let mut zero = 0u32;
		assert_eq!(zero.shift_right(), 0);
		assert_eq!(zero, 0);

		let mut single_digit = 7u32;
		assert_eq!(single_digit.shift_right(), 7);
		assert_eq!(single_digit, 0);

		let mut multi_digit = 12345u32;
		assert_eq!(multi_digit.shift_right(), 5);
		assert_eq!(multi_digit, 1234);
		assert_eq!(multi_digit.shift_right(), 4);
		assert_eq!(multi_digit, 123);
	}
}
